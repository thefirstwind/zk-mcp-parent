...
CREATE TABLE `zk_dubbo_method_parameter` (
  ...
  `parameter_schema_json` text COLLATE utf8mb4_unicode_ci COMMENT '参数结构化Schema（JSON，支持嵌套对象/集合/Map，用于生成MCP inputSchema）',
  `gmt_created` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
...
CREATE TABLE `zk_dubbo_service_method` (
  ...
  `method_description` text COLLATE utf8mb4_unicode_ci COMMENT '方法描述（人工维护）',
  `gmt_created` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `gmt_modified` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
